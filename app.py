...
# Replace your existing evaluate_recommender_enhanced function with this optimized version:

@st.cache_data
def evaluate_recommender_enhanced(train_rules, test_user_games_dict, fallback_map, k_values=[5, 10, 20]):
    """Evaluate the enhanced recommender system - OPTIMIZED VERSION"""
    ...
    # REDUCED sample size for faster evaluation
    max_eval_users = min(300, len(eligible_test_users))  # Changed from 600 to 300
    ...
    # Create progress bar but update MUCH less frequently
    progress_bar = st.progress(0)
    status_text = st.empty()
    update_frequency = max(1, len(eval_user_ids) // 10)  # Only update 10 times total instead of every user
    
    for i, user_id in enumerate(eval_user_ids):
        # CRITICAL CHANGE: Only update progress occasionally, not every iteration
        ...
        for k in k_values:
            try:
                # CRITICAL CHANGE: Use the simpler function for speed
